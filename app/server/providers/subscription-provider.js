...
export class SubscriptionProvider extends OpenAIProvider {
  ...
  async claudeTurn(args, session) {
    try {
      ...
      const stream = client.messages.stream({ model: this.model, max_tokens: session ? 1400 : 1200, system: args.prototypeContext ? prototypePrompt(SYSTEM_PROMPT, args) : args.researchContext ? researchPrompt(SYSTEM_PROMPT, args) : args.synthesisContext ? synthesisPrompt(SYSTEM_PROMPT, args) : session ? sessionContextBlock(args) : contextBlock(args), messages: buildMessages(args.transcript, args.message), tools: args.prototypeContext ? [PROTOTYPE_TOOL, OFFER_CHOICES_TOOL] : args.researchContext ? [RESEARCH_TOOL, OFFER_CHOICES_TOOL] : args.synthesisContext ? [SYNTHESIS_TOOL, OFFER_CHOICES_TOOL] : session ? [STAGE_CANDIDATES_TOOL, SUGGEST_INQUIRY_TOOL, OFFER_CHOICES_TOOL] : [STAGE_CANDIDATES_TOOL, OFFER_CHOICES_TOOL] });
      stream.on('text', text => args.onToken?.(text));
      const final = await stream.finalMessage();
      const reply = final.content.filter(b => b.type === 'text').map(b => b.text).join('');
      if (args.prototypeContext) return prototypeResult(reply, final.content.filter(b => b.type === 'tool_use'), args.prototypeContext);
      if (args.researchContext) return researchResult(reply, final.content.filter(b => b.type === 'tool_use'), args.researchContext);
