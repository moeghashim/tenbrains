...
type TicketPlan = { criteria: string[]; checklist: string[] };
...
type ApiCandidate =
  | ({ id: string; type: 'ticket-plan'; ticketId: string; stagedAfter: string } & TicketPlan)
  | { id: string; type: 'fog-retirement'; questionId: string; reason: string; evidence?: string[]; stagedAfter: string }
...
type ApiSession = {
  id: string;
  type: 'grilling' | 'synthesis' | 'research' | 'prototype';
...
function MapOverview({
  ...
  async function startSession(ticket: Ticket & { id?: string }) {
    ...
    const response = await fetch(`/api/discoveries/${liveDiscovery.id}/sessions`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ ticketId: ticket.id, type: ticket.type === 'Prototype' ? 'prototype' : ticket.type === 'Research' ? 'research' : ticket.type === 'Synthesis' ? 'synthesis' : 'grilling' }),
    });
              ...
              <section className="map-grid" aria-label="Decision map">
                <MapColumn title="Open Frontier" count={displayedTickets.length} tone="frontier" active={segment === 'frontier'}>
                  ...
                  {displayedTickets.map((ticket) => (
                    <TicketCard
                      key={ticket.title}
                      ticket={{ ...ticket, selected: isDemoMap ? ticket.selected : true }}
    ...
    <Card className={`ticket-card ${ticket.selected ? 'selected' : ''}`}>
      <CardContent className="ticket-content">
        ...
        <div className="card-actions">
          {live && ticket.plan && <span className="ticket-plan-indicator">Plan · {ticket.plan.criteria.length} {ticket.plan.criteria.length === 1 ? 'criterion' : 'criteria'}</span>}
...
function candidateText(candidate: ApiCandidate, fog: ApiDiscovery['map']['fogOfWar'] = []) {
  if (candidate.type === 'ticket-plan') return 'Prototype plan';
...
function PlanDetails({ plan }: { plan: TicketPlan }) {
  return <div className="prototype-plan-details"><div><h3>Success criteria</h3><ul>{plan.criteria.map((text, index) => <li key={index}>{text}</li>)}</ul></div><div><h3>Build checklist</h3><ol>{plan.checklist.map((text, index) => <li key={index}>{text}</li>)}</ol></div></div>;
}

function CandidateEvidence({ candidate }: { candidate: ApiCandidate }) {
  if ((candidate.type !== 'closed-decision' && candidate.type !== 'fog-retirement') || !candidate.evidence?.length) return null;
    ...
    <Dialog open={open} onOpenChange={(nextOpen) => { if (!approving) onOpenChange(nextOpen); }}>
      <DialogContent className="grid max-h-[92dvh] w-[calc(100%-24px)] grid-rows-[auto_minmax(0,1fr)_auto] gap-0 overflow-hidden p-0 sm:max-w-2xl">
        ...
        <ScrollArea className="min-h-0">
          <div className="space-y-3 p-5">
            {candidates.map((candidate) => {
              ...
              const helper = candidate.type === 'ticket-plan' ? 'Attach this plan to the Open Frontier ticket.' : candidate.type === 'fog-retirement' ? `Fog of War retirement · ${candidate.reason}` : candidate.type === 'ticket'
              ...
              return (
                ...
                />
                {candidate.type === 'ticket-plan' && <PlanDetails plan={candidate} />}<CandidateEvidence candidate={candidate} />
                </div>
              );
            })}
            {error && (
        ...
        <DialogFooter className="mx-0 mb-0 border-t border-border bg-[var(--surface-subtle)] px-5 py-3 sm:justify-between">
          <Button variant="outline" disabled={approving} onClick={() => onOpenChange(false)}>{prototype ? 'Continue prototype' : research ? 'Continue research' : synthesis ? 'Continue synthesis' : 'Continue grilling'}</Button>
...
function LiveEvidenceView({ navigate }: { navigate: (path: string) => void }) {
  ...
  const liveSessions = discovery?.sessions.filter((item): item is ApiSession => (item.type === 'grilling' || item.type === 'synthesis' || item.type === 'research' || item.type === 'prototype')) ?? [];
            ...
            {status === 'ready' && groups.map(({ session, records }) => (
              <section className="evidence-session-group" key={session.id} aria-labelledby={`evidence-session-${session.id}`}>
                <div className="evidence-session-header">
                  <div>
                    <p className="eyebrow">{session.type === 'prototype' ? 'Prototype' : session.type === 'research' ? 'Research' : session.type === 'synthesis' ? 'Synthesis' : 'Grilling'} session</p>
...
function sessionMapData(session: ApiSession): SessionMapData {
  ...
  return {
    ...
    candidateUpdates: session.staged.map((candidate) => ({
      id: candidate.id,
      type: candidate.type === 'ticket-plan' ? 'open-frontier-ticket' : candidate.type === 'ticket' ? 'open-frontier-ticket' : candidate.type,
      ...
      evidenceIds: candidate.type === 'closed-decision' || candidate.type === 'fog-retirement' ? (session.type !== 'grilling' ? candidate.evidence ?? [] : [...new Set([...(candidate.evidence ?? []), ...evidenceIds])]) : evidenceIds,
    })),
    outcome: {
  ...
  useEffect(() => {
    ...
    fetch(`/api/discoveries/${discoveryId}`)
      ...
      .then((discovery: ApiDiscovery | null) => {
        const found = discovery?.sessions.find((item): item is ApiSession => item.id === sessionId && (item.type === 'grilling' || item.type === 'synthesis' || item.type === 'research' || item.type === 'prototype'));
...
function SessionsList({ navigate }: { navigate: (path: string) => void }) {
  ...
  const sessions = discovery?.sessions.filter((item): item is ApiSession => (item.type === 'grilling' || item.type === 'synthesis' || item.type === 'research' || item.type === 'prototype')) ?? [];
  return (
    <div className="app-shell">
      <SideNav active="session" navigate={navigate} />
      <main className="main-surface workspace-main">
...
function LiveGrillingSession({ navigate, sessionId }: { navigate: (path: string) => void; sessionId: string }) {
  const [discovery, setDiscovery] = useState<ApiDiscovery | null>(null);
  const [message, setMessage] = useState('');
  const [findingRecorded, setFindingRecorded] = useState(false);
  const [streamedReply, setStreamedReply] = useState('');
  const [streamedChoices, setStreamedChoices] = useState<WayfinderChoice[]>([]);
  const [working, setWorking] = useState(false);
  const [reviewOpen, setReviewOpen] = useState(false);
  ...
  const discoveryId = window.sessionStorage.getItem('ten-brains-live-discovery');
  const session = discovery?.sessions.find((item): item is ApiSession => item.id === sessionId && (item.type === 'grilling' || item.type === 'synthesis' || item.type === 'research' || item.type === 'prototype')) ?? null;
  ...
  async function refresh(): Promise<ApiSession> {
    ...
    const updated: ApiDiscovery = await response.json();
    const found = updated.sessions.find((item): item is ApiSession => item.id === sessionId && (item.type === 'grilling' || item.type === 'synthesis' || item.type === 'research' || item.type === 'prototype'));
  ...
  function updateLiveSession(update: (current: ApiSession) => ApiSession) {
    setDiscovery((current) => current ? {
      ...current,
      sessions: current.sessions.map((item) => item.id === sessionId && (item.type === 'grilling' || item.type === 'synthesis' || item.type === 'research' || item.type === 'prototype') ? update(item) : item),
    } : current);
  }

  async function sendSessionMessage(textOverride?: string, choiceLabel?: string, isFinding = false) {
    const rawText = textOverride ?? message;
    const text = isFinding ? rawText : rawText.trim();
    if (!text.trim() || !discoveryId || !session || working) return;
    setError(null);
    setFindingRecorded(false);
    setWorking(true);
    setStreamedReply('');
    setStreamedChoices([]);
    ...
    try {
      const response = await fetch(`/api/discoveries/${discoveryId}/sessions/${session.id}/messages`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ message: text, ...(choiceLabel ? { choiceLabel } : {}), ...(isFinding ? { isFinding: true } : {}) }),
      });
      ...
      while (true) {
        ...
        for (const block of blocks) {
          ...
          if (event === 'token') {
            ...
            updateLiveSession((current) => ({
              ...current,
              evidence: [...current.evidence.filter((item) => item.id !== data.id), data],
              transcript: current.transcript.map((turn) => turn.id === 'local-pending-session-turn' ? { ...turn, id: data.sourceTurn, text: data.text } : turn),
            }));
          } else if (event === 'candidate') {
      ...
      await refresh();
      setMessage((current) => current === rawText ? '' : current);
      setStreamedReply('');
  ...
  return (
    <div className={`app-shell session-shell live-session-shell ${session.type === 'research' || session.type === 'prototype' ? 'research-session' : ''} ${session.type === 'prototype' ? 'prototype-session' : ''}`}>
      ...
      <main className="main-surface session-main">
        ...
        {session.type !== 'grilling' && <div className="synthesis-session-type"><Badge variant="outline">{session.type === 'prototype' ? 'Prototype' : session.type === 'research' ? 'Research' : 'Synthesis'} · You + Wayfinder</Badge></div>}
        ...
        {session.type === 'prototype' && discovery?.map.openFrontier.find((ticket) => ticket.id === session.ticketId)?.plan && <section className="approved-prototype-plan" aria-label="Approved Prototype plan"><p className="eyebrow">Approved plan · Build outside Ten Brains, then record the result.</p><PlanDetails plan={discovery.map.openFrontier.find((ticket) => ticket.id === session.ticketId)!.plan!} /></section>}
        ...
        <section className="session-grid live-session-grid">
          ...
          <PanelShell title="Live transcript">
            ...
            <LiveSessionComposer prototype={session.type === 'prototype'} research={session.type === 'research' || session.type === 'prototype'} findingRecorded={findingRecorded} onRecordFinding={() => void sendSessionMessage(undefined, undefined, true)} message={message} working={working} momentStatus={momentStatus} markDisabled={!lastUserTurn || lastTurnMarked} error={error} onMessageChange={(value) => { setMessage(value); if (error?.kind === 'stream') setError(null); }} onSend={() => sendSessionMessage()} onMark={markMoment} onRetry={retry} />
          </PanelShell>
          {signals}
        </section>
      </main>
  ...
  return (
    <PanelShell title="Session Signals" drawer={drawer}>
      <div className="signal-section">
        ...
        {session.evidence.length === 0 && <p className="live-signal-empty">{session.type === 'prototype' ? 'Record a result to add verbatim evidence.' : session.type === 'research' ? 'Record a finding to add verbatim evidence.' : session.type === 'synthesis' ? 'Wayfinder uses evidence from all sessions. Capture an example in a Grilling session if none exists.' : 'Mark a concrete moment to add evidence.'}</p>}
        {session.evidence.map((item) => <Card className="evidence-item live-arrival" key={item.id}><CardContent className="evidence-content live-evidence-content"><Bookmark aria-hidden="true" /><span><strong>{item.id}</strong>{(session.type === 'research' || session.type === 'prototype') && <span className="verbatim-label">{session.type === 'prototype' ? 'Verbatim result' : 'Verbatim finding'}</span>}<span className="captured-evidence-text">{item.text}</span></span></CardContent></Card>)}
      </div>
      <div className="signal-section map-updates">
        ...
        {session.staged.map((candidate) => <div className="live-arrival" key={candidate.id}><CandidateUpdate tone={candidate.type === 'fog-question' || candidate.type === 'fog-retirement' ? 'fog' : candidate.type === 'closed-decision' ? 'decision' : 'frontier'} label={candidate.type === 'ticket-plan' ? 'Candidate Prototype plan' : candidate.type === 'fog-retirement' ? 'Retire Fog of War question' : candidate.type === 'fog-question' ? 'Candidate Fog of War question' : candidate.type === 'closed-decision' ? 'Candidate Closed Decision' : candidate.type === 'destination-draft' ? 'Destination refinement' : 'Candidate Open Frontier ticket'} text={candidateText(candidate, fog)} />{candidate.type === 'fog-retirement' && <p className="candidate-retirement-reason">{candidate.reason}</p>}{candidate.type === 'ticket-plan' && <PlanDetails plan={candidate} />}<CandidateEvidence candidate={candidate} /></div>)}
...
function LiveSessionComposer({ prototype = false, research = false, findingRecorded = false, onRecordFinding, message, working, momentStatus, markDisabled, error, mobile = false, onMessageChange, onSend, onMark, onRetry }: { prototype?: boolean; research?: boolean; findingRecorded?: boolean; onRecordFinding?: () => void; message: string; working: boolean; momentStatus: 'idle' | 'saving' | 'saved'; markDisabled: boolean; error: LiveSessionError | null; mobile?: boolean; onMessageChange: (message: string) => void; onSend: () => void; onMark: () => void; onRetry: () => void }) {
  return (
    <div className={`${mobile ? 'live-mobile-composer' : 'composer live-session-composer'} ${research ? 'research-composer' : ''}`}>
      {research ? <textarea aria-label="Send to Wayfinder" placeholder={prototype ? "Ask a question or paste a result." : "Ask a question or paste a finding."} value={message} disabled={working} onChange={(event) => onMessageChange(event.target.value)} /> : <Input aria-label="Send to Wayfinder" placeholder="Give Wayfinder one concrete example." value={message} disabled={working} onChange={(event) => onMessageChange(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') onSend(); }} />}
      {!mobile && !research && <Button variant="outline" size="sm" disabled={working || markDisabled || momentStatus === 'saving'} onClick={onMark}>{momentStatus === 'saving' ? <LoaderCircle aria-hidden="true" className="animate-spin" /> : momentStatus === 'saved' ? <CheckCircle2 aria-hidden="true" /> : <Bookmark aria-hidden="true" />}{momentStatus === 'saving' ? 'Saving moment' : momentStatus === 'saved' ? 'Moment saved' : 'Mark moment'}</Button>}
      {research && <Button variant="outline" size="sm" disabled={working || !message.trim()} onClick={onRecordFinding}>{prototype ? 'Record result' : 'Record finding'}</Button>}
      ...
      {research && <p className="finding-status" role="status">{findingRecorded ? `${prototype ? 'Result' : 'Finding'} recorded verbatim. Retry sends chat without recording again.` : `Send asks Wayfinder. Record ${prototype ? 'result' : 'finding'} saves your text verbatim as evidence.`}</p>}
