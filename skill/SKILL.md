...
description-token overlap come first, with interest/familiarity ratings breaking ties. Inspect
`objective show <slug>` for grouped records and raw `data.progress` counts covering accounts,
analyzed posts/transcripts, saved bookmarks, tracks completed, and learning days completed/total.
Do not turn those counts into a made-up percentage.
